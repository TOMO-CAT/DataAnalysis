...
class MovieAnalysis():
    ...
    def movie_info(self, movie):
        ...
        #路径不能出现中文字符
        path = 'C:\\Users\\YANG\\Desktop\\ds\\data'
        csv_file = path + '\\' + movie + '.csv'
        #注意csv默认不保存utf-8，需要自己用记事本打开更改编码为utf-8, 否则会报错UnicodeDecodeError
    ...
    def draw_content_pic(self, movie):
        ...
        for indexs in dta.index:  
            score_list.append(tuple(dta.loc[indexs].values[:])) 
        ...
        for i in list(info_new['date']):
            location = info_new[(info_new.date==i)&(info_new.score==50)].index.tolist()
            if location == []:
                creat_df.loc[mark] = [50, i, 0]
                mark += 1
            location = info_new[(info_new.date==i)&(info_new.score==40)].index.tolist()
            if location == []:
                creat_df.loc[mark] = [40, i, 0]
                mark += 1
            location = info_new[(info_new.date==i)&(info_new.score==30)].index.tolist()
            if location == []:
                creat_df.loc[mark] = [30, i, 0]
                mark += 1
            location = info_new[(info_new.date==i)&(info_new.score==20)].index.tolist()
            if location == []:
                creat_df.loc[mark] = [20, i, 0]
                mark += 1
            location = info_new[(info_new.date==i)&(info_new.score==10)].index.tolist()
            if location == []:
                creat_df.loc[mark] = [10, i, 0]
                mark += 1
        ##创建了一个空的creat_df
        ...
        for i in attr:
            v1.append(int(info_new[(info_new['date']==i)&(info_new['score']==50)]['votes']))
            v2.append(int(info_new[(info_new['date']==i)&(info_new['score']==40)]['votes']))
            v3.append(int(info_new[(info_new['date']==i)&(info_new['score']==30)]['votes']))
            v4.append(int(info_new[(info_new['date']==i)&(info_new['score']==20)]['votes']))
            v5.append(int(info_new[(info_new['date']==i)&(info_new['score']==10)]['votes']))
        #pyecharts绘图
        line = Line("影评走势图")
        ...
        line.add("一星", attr, v5, is_stack=True, is_convert=False, mark_line=["average"], is_more_utils=True)
    ...
    def count_sentiment(csv_file):
        ...
        d = pd.read_csv(csv_file, encoding='utf-8') ##注意csv默认不保存utf-8，需要自己用记事本打开更改编码为utf-8
        motion_list = [] #构造空列表，相当于向量
        for i in d['content']:
            try:  
                ##文本过长时提取关键词后得分比较准确，而且SnowNLP这主要由于此SnowNLP主要用贝叶斯机器学习方法进行训练文本，机器学习在语料覆盖上不够，特征上工程处理不当会减分，也没考虑语义等           
                s = round(SnowNLP("".join(jieba.analyse.textrank(i))).sentiments, 2)
                motion_list.append(s)    #将每一步的结果追加到motion向量中
            except :
                continue     ##python异常处理
        result = {}
        for i in set(motion_list):
            result[i] = motion_list.count(i)    #将每一步追加到motion_list列表中，每个sentiment得分的计数
        return result
